...
These principles combine Anaplan's Modeling Experience guidance with the Planual standards for model building:

1. **Business case first**: Define the planning process, decisions, facts, lists, users, time ranges, and versions before building or automating.
2. **DISCO separation**: Keep Data, Input, System, Calculation, and Output concerns in separate modules and functional areas.
3. **Central Library discipline**: Treat lists, subsets, time, versions, users, roles, and line item subsets as shared architecture. Reuse governed structures instead of creating one-off integration artefacts.
4. **Right-size dimensionality**: Apply only the dimensions a line item needs. Use subsets, line item applies-to, and time ranges to control cell count.
5. **Simple, auditable formulas**: Break complex logic into clear intermediate line items, store mappings in system modules, and avoid brittle hard-coded references.
6. **Governed integrations**: Prefer saved views, import/export actions, and processes designed for integration. Validate mapping, task status, rejected rows, and downstream outputs.
7. **Production safety**: Respect ALM, selective access, production data, current period, switchover, and delete actions. Structural changes need model-builder governance.
