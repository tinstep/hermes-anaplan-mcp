...
// ============================================================
// DESIGN SYSTEM B — Classic Executive (Navy + Serif Titles)
// HYBRID MARGINS: 1.0" cover, 0.4" content slides (matching reference)
// ============================================================
const S = {
  // Colors
  deepNavy: "003A70",
  ...
  // Layout — Content slides: 0.5" margins
  ML: 0.5,
  MR: 0.5,
  CW: 12.333,  // 13.333 - 0.5 - 0.5
  TY: 0.4,
  TH: 0.8,
  STY: 1.4,
  STH: 0.4,
  ...
  // Cover slide: 1.0" margins
  COVER_ML: 1.0,
  COVER_MT: 1.0,
  COVER_CW: 11.333, // 13.333 - 1.0 - 1.0
...
function getMonthlyIndices(fyLabel) {
  ...
  const result = [];
  qLabels.forEach((ql) => {
    const qi = columns.indexOf(ql);
    if (qi >= 0) result.push(qi - 3, qi - 2, qi - 1);
  });
  return result;
}

function getMonthly(fyLabel, rowName) {
  return getMonthlyIndices(fyLabel).map((i) => dataRows[rowName][i] || 0);
}

function getQuarterly(fyLabel, rowName) {
  const m = getMonthly(fyLabel, rowName);
  return [sum(m.slice(0,3)), sum(m.slice(3,6)), sum(m.slice(6,9)), sum(m.slice(9,12))];
}

const monthly = {
  FY24: getMonthly("FY24", "Historical Sales"),
  FY25: getMonthly("FY25", "Historical Sales"),
  FY26: getMonthly("FY26", "Forecast"),
  FY27: getMonthly("FY27", "Forecast"),
};
const annual = {
...
const yoy2526 = ((annual.FY26 - annual.FY25) / annual.FY25 * 100).toFixed(1);
const yoy2627 = ((annual.FY27 - annual.FY26) / annual.FY26 * 100).toFixed(1);

// ============================================================
// HELPERS (adjusted for tight margins)
// ============================================================
const makeShadow = () => ({
...
function addTitle(slide, text) {
  slide.addText(text, {
    ...
    fontSize: S.FT, bold: true, color: S.deepNavy, fontFace: S.FONT_TITLE,
  });
}
...
function addDivider(slide) {
  slide.addShape("rect", {
    ...
    fill: { color: S.deepNavy },
  });
}

function addSubtitle(slide, text) {
  slide.addText(text, {
    x: S.ML, y: S.STY, w: S.CW, h: S.STH,
    fontSize: S.FST, color: S.mediumGray, fontFace: S.FONT_BODY,
  });
}

function addBumper(slide, text, highlight = false) {
  slide.addShape("rect", {
    x: S.ML, y: 6.35, w: S.CW, h: 0.45,
    fill: { color: highlight ? "E8F0FF" : S.lightGray },
  });
  slide.addShape("rect", {
    x: S.ML, y: 6.35, w: 0.08, h: 0.45,
    fill: { color: S.deepNavy },
  });
  slide.addText(text, {
    x: S.ML + 0.2, y: 6.45, w: S.CW - 0.2, h: 0.25,
    fontSize: 13, bold: true, color: S.charcoal, fontFace: S.FONT_BODY,
  });
}

function addFooter(slide, source, pageNum) {
  slide.addText(source, {
    x: S.ML, y: S.FY, w: 9, h: 0.3,
    fontSize: S.FF, color: S.mediumGray, fontFace: S.FONT_BODY,
  });
  slide.addText(String(pageNum), {
    x: S.ML + S.CW - 0.5, y: S.FY, w: 0.5, h: 0.3,
    fontSize: S.FF, color: S.mediumGray, fontFace: S.FONT_BODY, align: "right",
  });
}

function addInsightPanel(slide, title, insights) {
  const panelX = 9.2;   // shifted right with wider content area
  const panelW = 3.5;
  // Card background
  slide.addShape("rect", {
    x: panelX, y: S.BY, w: panelW, h: S.BH,
    fill: { color: S.lightGray },
    rectRadius: 0.1,
    shadow: makeShadow(),
  });
  // Left accent
  slide.addShape("rect", {
    x: panelX, y: S.BY, w: 0.06, h: S.BH,
    fill: { color: S.deepNavy },
  });
  // Title
  slide.addText(title, {
    x: panelX + 0.3, y: S.BY + 0.2, w: panelW - 0.5, h: 0.4,
    fontSize: 16, bold: true, color: S.deepNavy, fontFace: S.FONT_TITLE,
  });
  // Insight items
  insights.forEach((ins, i) => {
    const iy = S.BY + 0.8 + i * 1.05;
    slide.addText(ins.label, {
      x: panelX + 0.3, y: iy, w: panelW - 0.5, h: 0.25,
      fontSize: 11, bold: true, color: S.charcoal, fontFace: S.FONT_BODY,
    });
    slide.addText(ins.value, {
      x: panelX + 0.3, y: iy + 0.25, w: panelW - 0.5, h: 0.3,
      fontSize: 18, bold: true,
      color: ins.value.startsWith("+") ? S.teal : (ins.value.startsWith("-") ? S.accentRed : S.deepNavy),
      fontFace: S.FONT_BODY,
    });
    slide.addText(ins.note, {
      x: panelX + 0.3, y: iy + 0.55, w: panelW - 0.5, h: 0.3,
      fontSize: 10, color: S.mediumGray, fontFace: S.FONT_BODY,
    });
  });
}
...
const chartDefaults = {
  ...
  catAxisLabelFontFace: S.FONT_BODY,
  valAxisLabelFontFace: S.FONT_BODY,
  valGridLine: { color: "E0E0E0", size: 0.5 },
  catGridLine: { style: "none" },
  dataLabelColor: S.charcoal,
  dataLabelFontSize: 10,
  dataLabelFontFace: S.FONT_BODY,
};
...
pres.title = "FY26 Sales Forecast - Laptop Pro - Acme Corporation (Hybrid Margins)";

// --- SLIDE 1: COVER (1" margins all around) ---
{
  const sl = pres.addSlide();
  sl.background = { color: S.deepNavy };

  sl.addText("FY26 Sales Forecast", {
    x: S.COVER_ML, y: 2.0, w: S.COVER_CW, h: 1.2,
    fontSize: 44, bold: true, color: S.white, fontFace: S.FONT_TITLE,
  });
  sl.addText("Laptop Pro  |  Acme Corporation", {
    x: S.COVER_ML, y: 3.2, w: S.COVER_CW, h: 0.6,
    fontSize: 22, color: "C9A94E", fontFace: S.FONT_BODY,
  });

  // Divider
  sl.addShape("rect", { x: S.COVER_ML, y: 4.0, w: 4.0, h: 0.03, fill: { color: S.white } });

  sl.addText("Sales & Planning Team Review  |  February 2026", {
    x: S.COVER_ML, y: 4.3, w: S.COVER_CW, h: 0.5,
    fontSize: 14, color: S.chartGray, fontFace: S.FONT_BODY,
  });
}
...
// --- SLIDE 2: EXECUTIVE SUMMARY ---
{
  ...
  cards.forEach((c, i) => {
    ...
    // Top accent
    sl.addShape("rect", {
      x: cx, y: S.BY, w: cardW, h: 0.06,
      fill: { color: i === 1 ? S.teal : S.deepNavy },
    });
    // Title
    sl.addText(c.title, {
      x: cx + 0.3, y: S.BY + 0.25, w: cardW - 0.6, h: 0.35,
      fontSize: 14, bold: true, color: S.mediumGray, fontFace: S.FONT_BODY,
    });
    // Value
    sl.addText(c.value, {
      x: cx + 0.3, y: S.BY + 0.6, w: cardW - 0.6, h: 0.8,
      fontSize: 40, bold: true, color: S.charcoal, fontFace: S.FONT_TITLE,
    });
    // Sub
    sl.addText(c.sub, {
      x: cx + 0.3, y: S.BY + 1.35, w: cardW - 0.6, h: 0.3,
      fontSize: 13, color: S.mediumGray, fontFace: S.FONT_BODY,
    });
    // Delta
    sl.addText(c.delta, {
      x: cx + 0.3, y: S.BY + 1.75, w: cardW - 0.6, h: 0.35,
      fontSize: 16, bold: true, color: c.deltaColor, fontFace: S.FONT_BODY,
    });
  });

  // Key insight
  sl.addText([
    { text: "Key Insight: ", options: { bold: true, color: S.deepNavy } },
    ...
    x: S.ML, y: 5.3, w: S.CW, h: 0.7,
    fontSize: 13, fontFace: S.FONT_BODY,
  });
...
// --- SLIDE 3: ANNUAL TREND (BAR + INSIGHT SIDEBAR) ---
{
  const sl = pres.addSlide();
  sl.background = { color: S.white };
  ...
  sl.addChart(pres.charts.BAR, [{
    ...
    x: S.ML, y: S.BY, w: 8.5, h: S.BH,
    barDir: "col",
    chartColors: [S.deepNavy],
    ...chartDefaults,
    showValue: true,
...
// --- SLIDE 4: QUARTERLY COMPARISON (GROUPED BAR) ---
{
  ...
  sl.addChart(pres.charts.BAR, [
    ...
    x: S.ML, y: S.BY, w: S.CW, h: 4.2,
    barDir: "col",
    barGrouping: "clustered",
    chartColors: [S.chartGray, S.deepNavy, S.teal, S.accentBlue],
    ...chartDefaults,
    showValue: true,
    dataLabelFontSize: 9,
    showLegend: true,
...
// --- SLIDE 5: MONTHLY DETAIL (LINE CHART) ---
{
  const sl = pres.addSlide();
  sl.background = { color: S.white };
  ...
  sl.addChart(pres.charts.LINE, [
    ...
    x: S.ML, y: S.BY, w: S.CW, h: 4.2,
    chartColors: [S.chartGray, S.deepNavy, S.teal],
    lineSize: 2.5,
    lineSmooth: false,
    ...chartDefaults,
    showValue: true,
...
// --- SLIDE 6: FORECAST METHODOLOGY ---
{
  ...
  const methods = [
    {
      ...
      formula: "Historical Sales (t-24) × 1.5",
  ...
  methods.forEach((m, i) => {
    ...
    // Left accent
    sl.addShape("rect", {
      x: S.ML, y: cy, w: 0.08, h: 1.35,
      fill: { color: m.color },
    });
    // Method name
    sl.addText(m.name, {
      x: S.ML + 0.3, y: cy + 0.1, w: 6.0, h: 0.3,
      fontSize: 16, bold: true, color: S.charcoal, fontFace: S.FONT_TITLE,
    });
    // Formula
    sl.addText(m.formula, {
      x: S.ML + 0.3, y: cy + 0.4, w: 6.0, h: 0.25,
      fontSize: 12, italic: true, color: S.deepNavy, fontFace: S.FONT_BODY,
    });
    // Description
    sl.addText(m.desc, {
      x: S.ML + 0.3, y: cy + 0.7, w: 8.0, h: 0.4,
      fontSize: 11, color: S.mediumGray, fontFace: S.FONT_BODY,
    });
    // FY26 value
    sl.addText(m.fy26, {
      x: 10.5, y: cy + 0.15, w: 2.4, h: 0.55,
      fontSize: 28, bold: true, color: m.color, fontFace: S.FONT_TITLE, align: "center",
    });
    sl.addText("FY26", {
      x: 10.5, y: cy + 0.75, w: 2.4, h: 0.25,
      fontSize: 11, color: S.mediumGray, fontFace: S.FONT_BODY, align: "center",
    });
  });
...
// --- SLIDE 7: MONTHLY DATA TABLE ---
{
  ...
  const headerRow = [
    { text: "", options: { fill: { color: S.deepNavy }, color: S.white, bold: true, fontSize: 10, fontFace: S.FONT_BODY, align: "center" } },
    ...months.map((m) => ({ text: m, options: { fill: { color: S.deepNavy }, color: S.white, bold: true, fontSize: 10, fontFace: S.FONT_BODY, align: "center" } })),
    { text: "Total", options: { fill: { color: S.deepNavy }, color: S.white, bold: true, fontSize: 10, fontFace: S.FONT_BODY, align: "center" } },
  ];
  ...
  const makeRow = (label, data, total, bgColor) => [
    ...
    ...data.map((v) => ({ text: String(v), options: { fill: { color: bgColor }, fontSize: 10, fontFace: S.FONT_BODY, color: S.charcoal, align: "right" } })),
    { text: String(total), options: { fill: { color: bgColor }, bold: true, fontSize: 10, fontFace: S.FONT_BODY, color: S.charcoal, align: "right" } },
  ];
  ...
  const tableRows = [
    ...
    [
      { text: "FY26 YoY %", options: { fill: { color: S.white }, bold: true, fontSize: 10, fontFace: S.FONT_BODY, color: S.deepNavy, italic: true } },
      ...yoyGrowth.map((v) => ({
        text: v,
        options: {
          fill: { color: S.white }, fontSize: 10, fontFace: S.FONT_BODY, align: "right", italic: true,
          color: v === "N/A" ? S.mediumGray : (v.startsWith("-") ? S.accentRed : S.teal),
        },
      })),
      { text: "+55%", options: { fill: { color: S.white }, bold: true, fontSize: 10, fontFace: S.FONT_BODY, color: S.teal, align: "right", italic: true } },
    ],
  ];

  // Wider columns with tight margins
  const colW = [1.2, ...Array(12).fill(0.87), 1.0]; // sums to ~12.64

  sl.addTable(tableRows, {
    x: S.ML, y: S.BY + 0.1, w: S.CW,
...
// --- SLIDE 8: RISKS & NEXT STEPS ---
{
  ...
  sl.addText("Key Risks", {
    x: S.ML, y: S.BY + 0.1, w: 5.8, h: 0.35,
    fontSize: 18, bold: true, color: S.accentRed, fontFace: S.FONT_TITLE,
  });
  ...
  risks.forEach((r, i) => {
    ...
    sl.addShape("rect", {
      x: S.ML, y: ry, w: 5.8, h: 1.05,
      fill: { color: S.lightGray },
      rectRadius: 0.08,
    });
    sl.addShape("rect", {
      x: S.ML, y: ry, w: 0.06, h: 1.05,
      fill: { color: S.accentRed },
    });
    sl.addText(r.risk, {
      x: S.ML + 0.25, y: ry + 0.08, w: 5.3, h: 0.3,
      fontSize: 14, bold: true, color: S.charcoal, fontFace: S.FONT_BODY,
    });
    sl.addText(r.detail, {
      x: S.ML + 0.25, y: ry + 0.42, w: 5.3, h: 0.5,
      fontSize: 11, color: S.mediumGray, fontFace: S.FONT_BODY,
    });
  });
  ...
  sl.addText("Recommended Actions", {
    x: 6.6, y: S.BY + 0.1, w: 6.3, h: 0.35,
    fontSize: 18, bold: true, color: S.deepNavy, fontFace: S.FONT_TITLE,
  });
  ...
  const actionHeader = [
    { text: "#", options: { fill: { color: S.deepNavy }, color: S.white, bold: true, fontSize: 10, fontFace: S.FONT_BODY, align: "center" } },
    { text: "Action", options: { fill: { color: S.deepNavy }, color: S.white, bold: true, fontSize: 10, fontFace: S.FONT_BODY } },
    { text: "Owner", options: { fill: { color: S.deepNavy }, color: S.white, bold: true, fontSize: 10, fontFace: S.FONT_BODY } },
    { text: "Target", options: { fill: { color: S.deepNavy }, color: S.white, bold: true, fontSize: 10, fontFace: S.FONT_BODY } },
  ];
  ...
  actions.forEach((a, i) => {
    ...
    actionRows.push(a.map((cell, j) => ({
      text: cell,
      options: { fill: { color: bg }, fontSize: 10, fontFace: S.FONT_BODY, color: S.charcoal, align: j === 0 ? "center" : "left" },
    })));
  });

  sl.addTable(actionRows, {
    x: 6.6, y: S.BY + 0.6, w: 6.3,
    border: { type: "solid", pt: 0.5, color: "CCCCCC" },
    colW: [0.4, 3.0, 1.5, 1.4],
    rowH: 0.42,
    valign: "middle",
  });
...
// === SAVE ===
const fileName = "FY26_Sales_Forecast_HYBRID_MARGINS.pptx";
