...
// DESIGN SYSTEM C — Clean & Approachable (Green dot motif, Open Sans)
const S = {
  brandGreen: "86BC24",
  black: "000000",
  charcoal: "333333",
...
function addTitle(slide, text) { slide.addText(text, { x: S.ML, y: S.TY, w: S.CW, h: S.TH, fontSize: S.FT, bold: true, color: S.brandGreen, fontFace: S.FONT_TITLE }); }
function addDivider(slide) { slide.addShape("rect", { x: S.ML, y: S.DIV_Y, w: S.CW, h: 0.03, fill: { color: S.brandGreen } }); }
function addBumper(slide, text) {
  slide.addShape("rect", { x: S.ML, y: S.BUMPER_Y, w: S.CW, h: S.BUMPER_H, fill: { color: S.lightGray } });
  slide.addShape("rect", { x: S.ML, y: S.BUMPER_Y, w: 0.08, h: S.BUMPER_H, fill: { color: S.brandGreen } });
...
function addInsightPanel(slide, title, insights) {
  ...
  slide.addShape("rect", { x: panelX, y: S.BY, w: 0.06, h: panelH, fill: { color: S.brandGreen } });
  slide.addText(title, { x: panelX + 0.3, y: S.BY + 0.2, w: panelW - 0.5, h: 0.4, fontSize: 16, bold: true, color: S.brandGreen, fontFace: S.FONT_TITLE });
  insights.forEach((ins, i) => {
    ...
    slide.addText(ins.value, { x: panelX + 0.3, y: iy + 0.25, w: panelW - 0.5, h: 0.3, fontSize: 18, bold: true, color: ins.value.startsWith("+") ? S.brandGreen : (ins.value.startsWith("-") ? S.accentRed : S.charcoal), fontFace: S.FONT_BODY });
...
pres.title = "FY26 Sales Forecast - Design System C";

// SLIDE 1: COVER
{
  const sl = pres.addSlide();
  sl.background = { color: S.charcoal };
  ...
  sl.addText("Laptop Pro  |  Acme Corporation", { x: S.COVER_ML, y: 3.2, w: S.COVER_CW, h: 0.6, fontSize: 22, color: S.brandGreen, fontFace: S.FONT_BODY });
...
// SLIDE 2: EXECUTIVE SUMMARY
{
  ...
  const cards = [
    ...
    { title: "FY26 Forecast", value: "18,300", sub: "units projected", delta: "+55.1% vs FY25", deltaColor: S.brandGreen },
  ...
  cards.forEach((c, i) => {
    ...
    sl.addShape("rect", { x: cx, y: S.BY, w: cw, h: 0.06, fill: { color: i === 1 ? S.brandGreen : S.charcoal } });
...
// SLIDE 3: ANNUAL TREND
{
  ...
  sl.addChart(pres.charts.BAR, [{ name: "Units", labels: ["FY24 (Actual)", "FY25 (Actual)", "FY26 (Forecast)", "FY27 (Forecast)"], values: [annual.FY24, annual.FY25, annual.FY26, annual.FY27] }], {
    x: S.ML, y: S.BY, w: 8.2, h: S.BH, barDir: "col", chartColors: [S.brandGreen], ...chartDefaults, showValue: true, showLegend: false,
  });
...
// SLIDE 4: QUARTERLY
{
  ...
  ], { x: S.ML, y: S.BY, w: S.CW, h: S.BH - 0.8, barDir: "col", barGrouping: "clustered", chartColors: [S.chartGray, S.charcoal, S.brandGreen, S.mediumGray], ...chartDefaults, showValue: true, showLegend: true, legendPos: "b", legendFontSize: 10, legendFontFace: S.FONT_BODY, legendColor: S.charcoal });
...
// SLIDE 5: MONTHLY LINE
{
  ...
  ], { x: S.ML, y: S.BY, w: S.CW, h: S.BH - 0.8, chartColors: [S.chartGray, S.charcoal, S.brandGreen], lineSize: 1.5, lineSmooth: false, ...chartDefaults, showValue: true, showLegend: true, legendPos: "b", legendFontSize: 10, legendFontFace: S.FONT_BODY, legendColor: S.charcoal, lineDataSymbol: "circle", lineDataSymbolSize: 3 });
...
// SLIDE 6: METHODOLOGY
{
  ...
  const methods = [
    ...
    { name: "Forecast — Previous Year", formula: "Historical Sales (t-12)", desc: "Uses prior year actuals as baseline forecast. Conservative approach.", fy26: "11,800", color: S.brandGreen },
  ...
  methods.forEach((m, i) => {
    ...
    sl.addText(m.formula, { x: S.ML + 0.3, y: cy + 0.4, w: 5.0, h: 0.25, fontSize: 12, italic: true, color: S.brandGreen, fontFace: S.FONT_BODY });
...
// SLIDE 7: DATA TABLE
{
  ...
  const headerRow = [
    ...
    ...months.map((m) => ({ text: m, options: { fill: { color: S.brandGreen }, color: S.white, bold: true, fontSize: 10, fontFace: S.FONT_BODY, align: "center" } })),
    { text: "Total", options: { fill: { color: S.brandGreen }, color: S.white, bold: true, fontSize: 10, fontFace: S.FONT_BODY, align: "center" } },
  ];
  ...
  const tableRows = [
    ...
    [
      { text: "FY26 YoY %", options: { fill: { color: S.white }, bold: true, fontSize: 10, fontFace: S.FONT_BODY, color: S.brandGreen, italic: true } },
      ...yoyGrowth.map((v) => ({ text: v, options: { fill: { color: S.white }, fontSize: 10, fontFace: S.FONT_BODY, align: "right", italic: true, color: v === "N/A" ? S.mediumGray : (v.startsWith("-") ? S.accentRed : S.brandGreen) } })),
      { text: "+55%", options: { fill: { color: S.white }, bold: true, fontSize: 10, fontFace: S.FONT_BODY, color: S.brandGreen, align: "right", italic: true } },
    ],
  ];
...
// SLIDE 8: RISKS & ACTIONS
{
  ...
  sl.addText("Recommended Actions", { x: 6.6, y: S.BY + 0.1, w: 5.7, h: 0.35, fontSize: 18, bold: true, color: S.brandGreen, fontFace: S.FONT_TITLE });
  ...
  const actionHeader = [
    { text: "#", options: { fill: { color: S.brandGreen }, color: S.white, bold: true, fontSize: 10, fontFace: S.FONT_BODY, align: "center" } },
    { text: "Action", options: { fill: { color: S.brandGreen }, color: S.white, bold: true, fontSize: 10, fontFace: S.FONT_BODY } },
    { text: "Owner", options: { fill: { color: S.brandGreen }, color: S.white, bold: true, fontSize: 10, fontFace: S.FONT_BODY } },
    { text: "Target", options: { fill: { color: S.brandGreen }, color: S.white, bold: true, fontSize: 10, fontFace: S.FONT_BODY } },
  ];
...
const fileName = "FY26_Sales_Forecast_SystemC.pptx";
