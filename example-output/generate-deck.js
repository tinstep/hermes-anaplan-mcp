...
const S = {
  ...
  // Layout — Content slides: 0.5" L/R/T, 0.2" bottom
  ML: 0.5,
  MR: 0.5,
  CW: 12.333,        // 13.333 - 0.5 - 0.5
  TY: 0.5,           // top margin
  TH: 0.8,           // title height
  DIV_Y: 1.35,       // TY + TH + 0.05
  BY: 1.5,           // body starts after divider
  FY: 7.1,           // 7.5 - 0.2(margin) - 0.2(footer height)
  BUMPER_H: 0.45,
  BUMPER_Y: 6.55,    // FY - 0.55
  BH: 4.95,          // BUMPER_Y - 0.1(gap) - BY = 6.45 - 1.5

  // Cover slide: 1.0" all around
  COVER_ML: 1.0,
...
function addDivider(slide) {
  slide.addShape("rect", {
    x: S.ML, y: S.DIV_Y, w: S.CW, h: 0.03,
...
function addSubtitle(slide, text) {
  slide.addText(text, {
    x: S.ML, y: S.DIV_Y + 0.1, w: S.CW, h: 0.4,
...
function addBumper(slide, text) {
  slide.addShape("rect", {
    x: S.ML, y: S.BUMPER_Y, w: S.CW, h: S.BUMPER_H,
    fill: { color: S.lightGray },
  });
  slide.addShape("rect", {
    x: S.ML, y: S.BUMPER_Y, w: 0.08, h: S.BUMPER_H,
    fill: { color: S.deepNavy },
  });
  slide.addText(text, {
...
function addInsightPanel(slide, title, insights) {
  const panelX = S.ML + S.CW - 3.6;  // right-aligned within content
  const panelW = 3.6;
  const panelH = S.BUMPER_Y - 0.1 - S.BY; // fits exactly above bumper
  const itemSpacing = (panelH - 0.8) / insights.length; // dynamic spacing

  // Card background
  slide.addShape("rect", {
    x: panelX, y: S.BY, w: panelW, h: panelH,
    fill: { color: S.lightGray },
    rectRadius: 0.1,
    shadow: makeShadow(),
  });
  // Left accent
  slide.addShape("rect", {
    x: panelX, y: S.BY, w: 0.06, h: panelH,
    fill: { color: S.deepNavy },
  });
  // Title
  slide.addText(title, {
  ...
  // Insight items — dynamically spaced to fit panel
...
// --- SLIDE 1: COVER (1.0" margins all around) ---
{
...
// --- SLIDE 2: EXECUTIVE SUMMARY ---
{
  ...
  sl.addText([
    ...
    x: S.ML, y: S.BY + 3.2, w: S.CW, h: 0.7,
...
// --- SLIDE 4: QUARTERLY COMPARISON (GROUPED BAR) ---
{
  ...
  sl.addChart(pres.charts.BAR, [
    ...
    x: S.ML, y: S.BY, w: S.CW, h: S.BH - 0.8,
    barDir: "col",
...
// --- SLIDE 7: MONTHLY DATA TABLE ---
{
  ...
  const colW = [1.2, ...Array(12).fill(0.86), 0.8]; // sums to ~12.32
...
// --- SLIDE 8: RISKS & NEXT STEPS ---
{
  ...
  sl.addText("Key Risks", {
    x: S.ML, y: S.BY + 0.1, w: 5.2, h: 0.35,
  ...
  risks.forEach((r, i) => {
    ...
    sl.addText(r.risk, {
      x: S.ML + 0.25, y: ry + 0.08, w: 4.7, h: 0.3,
      fontSize: 14, bold: true, color: S.charcoal, fontFace: S.FONT_BODY,
    });
    sl.addText(r.detail, {
      x: S.ML + 0.25, y: ry + 0.42, w: 4.7, h: 0.5,
      fontSize: 11, color: S.mediumGray, fontFace: S.FONT_BODY,
    });
  });
  ...
  sl.addTable(actionRows, {
    x: 6.6, y: S.BY + 0.6, w: 5.7,
    border: { type: "solid", pt: 0.5, color: "CCCCCC" },
    colW: [0.4, 2.5, 1.4, 1.4],
    rowH: 0.42,
    valign: "middle",
  });
...
// === SAVE ===
const fileName = "FY26_Sales_Forecast_Laptop_Pro_Acme_Corp.pptx";
